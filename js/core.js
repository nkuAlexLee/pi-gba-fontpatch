class ARMCore {
	constructor() {
		...
		// Dynamic font cracker: instruction-level callback injection
		this.instructionHook = null;
		this.instructionCallbacks = null;
		// Dynamic font cracker: callback can set this to skip original instruction
	...
	loadInstructionArm(address) {
		...
		next.mode = this.MODE_ARM;
		// Dynamic font cracker: inject callback if address matches
		if (this.instructionCallbacks && this.instructionCallbacks.has(address)) {
			var patch = this.instructionCallbacks.get(address);
			var cb = typeof patch === "function" ? patch : patch.callback;
			var originalOp = next;
			var cpu = this;
			next = function() {
				var result = cb ? cb(cpu, "arm", address >>> 0, originalOp) : null;
				if (result && result.skipOriginal) {
					return;
				}
				originalOp();
			};
			next.writesPC = (patch && typeof patch === "object" && patch.writesPC !== undefined)
				? !!patch.writesPC
				: originalOp.writesPC;
			next.fixedJump = (patch && typeof patch === "object" && patch.fixedJump !== undefined)
				? !!patch.fixedJump
				: originalOp.fixedJump;
			next.page = originalOp.page;
			next.address = address;
			next.opcode = instruction;
			next.mode = this.MODE_ARM;
		}
		if (this.instructionHook) {
			next = this.instructionHook(address >>> 0, next, "arm") || next;
		}
	...
	loadInstructionThumb(address) {
		...
		if (this.instructionCallbacks && this.instructionCallbacks.has(address)) {
			...
			var cpu = this;
			next = function() {
				var result = cb ? cb(cpu, "thumb", address >>> 0, originalOp) : null;
				if (result && result.skipOriginal) {
					return;
				}
				originalOp();
		...
		if (this.instructionHook) {
			next = this.instructionHook(address >>> 0, next, "thumb") || next;
		}
