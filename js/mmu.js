...
class GameBoyAdvanceMMU {
	constructor() {
		...
		// Dynamic font cracker: optional callbacks for runtime tracing
		this.vramStoreCallback = null;
		this.romReadCallback = null;
	...
	load32(offset) {
		var region = offset >>> this.BASE_OFFSET;
		if (
			this.romReadCallback &&
			(region === this.REGION_CART0 ||
				region === this.REGION_CART0 + 1 ||
				region === this.REGION_CART1 ||
				region === this.REGION_CART1 + 1 ||
				region === this.REGION_CART2 ||
				region === this.REGION_CART2 + 1)
		) {
			this.romReadCallback(offset >>> 0, 4);
		}
	...
	loadU8(offset) {
		...
		if (
			...
			this.romReadCallback(offset >>> 0, 1);
		}
	...
	loadU16(offset) {
		...
		if (
			...
			this.romReadCallback(offset >>> 0, 2);
		}
	...
	store8(offset, value) {
		...
		if (this.storeCallback) {
			this.storeCallback(offset >>> 0, value >>> 0, 1, region >>> 0);
		}
		if (this.vramStoreCallback && region === this.REGION_VRAM) {
			this.vramStoreCallback(offset >>> 0, value >>> 0, 1);
		}
	...
	store16(offset, value) {
		...
		var memory = this.memory[region];
		if (this.storeCallback) {
			this.storeCallback(offset >>> 0, value >>> 0, 2, region >>> 0);
		}
		if (this.vramStoreCallback && region === this.REGION_VRAM) {
			this.vramStoreCallback(offset >>> 0, value >>> 0, 2);
		}
	...
	store32(offset, value) {
		...
		var memory = this.memory[region];
		if (this.storeCallback) {
			this.storeCallback(offset >>> 0, value >>> 0, 4, region >>> 0);
		}
		if (this.vramStoreCallback && region === this.REGION_VRAM) {
			this.vramStoreCallback(offset >>> 0, value >>> 0, 4);
		}
